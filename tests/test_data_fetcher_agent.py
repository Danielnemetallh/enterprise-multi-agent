...
@pytest.fixture
def mock_ticket():
    return {"id": 1, "kunden_id": 1, "typ": "preisanfrage",
...
class TestRunDataFetcher:
    ...
    def test_preisanfrage_holt_preise_und_kunde(self, mock_llm, mock_ticket):
        mock_llm.return_value = "Cloud-Speicher Pro"

        result = run_data_fetcher("preisanfrage", mock_ticket)
    ...
    def test_beschwerde_holt_historie(self, mock_ticket):
        result = run_data_fetcher("beschwerde", mock_ticket)
        assert "kunde" in result
        assert "historie" in result
        assert "orders" in result["historie"]
        assert "tickets" in result["historie"]

    def test_kuendigung_holt_historie(self, mock_ticket):
        result = run_data_fetcher("kuendigung", mock_ticket)
        assert "kunde" in result
        assert "historie" in result

    def test_sonstiges_nur_kunde(self, mock_ticket):
        result = run_data_fetcher("sonstiges", mock_ticket)
        assert "kunde" in result
        assert "historie" not in result
        assert "preis_vergleich" not in result

    def test_kein_ticket(self):
        result = run_data_fetcher("preisanfrage", None)
